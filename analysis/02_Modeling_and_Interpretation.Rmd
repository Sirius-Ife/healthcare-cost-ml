...
**Plain language summary:** The tree-based models (random forest and XGBoost) typically outperform the linear (glmnet) model because they capture non-linear behavior and handle the skew/tail of cost better. In this case, they did out perform it with an R-squared of 0.9572316 and 0.9967845.
...
**Plain language:** log_next_cost, cost_change_pct and prev_year_cost are among the most important predictors.
...
I also show a SHAP summary plot (bar chart), feature 14 is log_next_cost.
...
**Plain language:** The model perfectly classified all cases (precision = 1, recall = 1), but an AUC of 0 indicates a calculation issue. In reality, the model likely overfit to the training data. Future iterations should ensure proper validation and recalculate AUC using predicted probabilities to assess true model performance.
...
**Plain language:** The top 500 members identified by the model represent a focused “high-risk” cohort. Within this group, the concentration of true high-cost members is 3.85 times higher than in the general population of 5,001 members.

In simpler terms, if we were to target just the 500 people the model flags as most likely to be high-cost, we’d be almost 4× more likely to find true high-cost cases than if we picked 500 people at random.
